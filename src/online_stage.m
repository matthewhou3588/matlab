...
anchor_unknown_estimated_dist = zeros(grid.num, 1)
for k = 1:grid.num
    anchor_unknown_estimated_dist(k,:) = 10^3*sqrt(sum((repmat(grid.points(k,:),1,1)-unknown_node.points).^2,2)).';  % the distance from the kth grid to all access points
end
